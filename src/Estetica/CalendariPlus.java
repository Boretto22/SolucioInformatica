...
public class CalendariPlus {
    ...
    public class Calendari {
        ...
        CalendariPlus cal, cPrev;

        // Botons del calendari
        DayButtons.DayButton[] buttons;
        ...
        public Calendari(int x, int y, int w, int h){
            ...
            this.cal = CalendariPlus.getInstance();
            cal.set(CalendariPlus.DAY_OF_MONTH, 1);

            this.any = cal.get(CalendariPlus.YEAR);
            this.mes = cal.get(CalendariPlus.MONTH) + 1;
            this.dia = cal.get(CalendariPlus.DATE);

            this.numDaysMonth = cal.getActualMaximum(CalendariPlus.DAY_OF_MONTH);

            this.dayOfWeek = cal.get(CalendariPlus.DAY_OF_WEEK);
            if(dayOfWeek== CalendariPlus.SUNDAY){ this.dayOfWeek = 6; }
            else { this.dayOfWeek  = this.dayOfWeek - 2; }

            cal.set(CalendariPlus.DAY_OF_WEEK, cal.getFirstDayOfWeek());
            this.firstDay = cal.get(CalendariPlus.DATE);

            cPrev = CalendariPlus.getInstance();
            setPrevCalendar(1, this.mes-2, this.any);

            this.numDaysPrevMonth = cPrev.getActualMaximum(CalendariPlus.DAY_OF_MONTH);
        ...
        public void setCalendar(int d, int m, int y){
            cal.set(CalendariPlus.YEAR, y);
            cal.set(CalendariPlus.MONTH, m);
            cal.set(CalendariPlus.DATE, d);
        }

        public void setPrevCalendar(int d, int m, int y){
            cPrev.set(CalendariPlus.YEAR, y);
            cPrev.set(CalendariPlus.MONTH, m);
            cPrev.set(CalendariPlus.DATE, d);
        }
